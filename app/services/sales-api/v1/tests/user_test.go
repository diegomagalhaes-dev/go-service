...
// dependencies for tests  while still providing a convenint syntas when
...
func Test_Users(t *testing.T) {
	...
	usrs, err := seed(context.Background(), api.User)
	if err != nil {
...
func (ut *UserTests) postUser201(t *testing.T) usergrp.AppUser {
	...
	email, err := mail.ParseAddress("johndoe@gmail.com")
	if err != nil {
...
func (ut *UserTests) putUser200(t *testing.T, id string) {
	u := usergrp.AppUpdateUser{
		Name: dbtest.StringPointer("John Doe"),
	}
	body, err := json.Marshal(&u)
	if err != nil {
		t.Fatal(err)
	...
	if ru.Name != "John Doe" {
		t.Fatalf("Should see an updated Name : got %q want %q", ru.Name, "John Doe")
	}
	...
	if ru.Email != email.Address {
		t.Fatalf("Should not affect other fields like Email : got %q want %q", ru.Email, "johndoe@gmail.com")
	}
}
